...
- Move long code, per-language variants and endpoint catalogues into `references/<topic>.md`, linked directly from `SKILL.md`. Keep new references one level deep — a reference file should not point to another reference file. (Inside the `yba-api` and `yba-terraform` sets, reference files link to their siblings today — `recipes.md` → `python-client.md`, `providers-universe.md` → `prereqs-and-network.md` and so on; `yba-terraform` does it deliberately, as a two-stage workflow. Both are grandfathered, not a pattern to copy. No reference file links into another *skill's* set, and none should.) Put a contents list at the top of any reference longer than about 100 lines so a partial read still shows its scope.
