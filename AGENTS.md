...
| References | every `references/…` path resolves — both markdown links and backtick-wrapped mentions like `` `references/foo.md` ``, ignoring fenced code so an illustrative path is not treated as a pointer (error); every reference file is linked or mentioned from `SKILL.md` (warn) |
