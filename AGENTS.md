...
- Move long code, per-language variants and endpoint catalogues into `references/<topic>.md`, linked directly from `SKILL.md`. Keep new references one level deep — a reference file should not point to another reference file. (Three sets already point a reference at one of its siblings and are grandfathered, not a pattern to copy: `yba-api` (`recipes.md` → `python-client.md`, `prometheus.md`), `yba-terraform` (`providers-universe.md` → `prereqs-and-network.md` and others, deliberately, as a two-stage workflow), and `yb-k8s-operator` (`workflows.md` → `kubeconfig-secrets.md`, as a plain-text instruction). The checker cannot see any of these — `RF001` resolves only paths named in `SKILL.md` — so this list is the whole record. No reference file points into another *skill's* set, and none should.) Put a contents list at the top of any reference longer than about 100 lines so a partial read still shows its scope.
...
- No exact version pins for dependencies. Name the package or coordinate (`psycopg-yugabytedb`, `com.yugabyte:r2dbc-postgresql`) and tell the agent to resolve that package's latest release from its registry at generation time. Do not describe the version format: the YugabyteDB drivers are separate packages, and registries differ — PyPI, NuGet and RubyGems publish plain version numbers, while Maven, npm, Go and crates.io use the upstream version plus a `-yb-N` qualifier — so a rule that filters on a suffix rejects valid releases. Compatibility constraints are fine when the reason is stated: a `>=` lower bound a library needs, or a `~>` range that avoids a release with known bugs. Calendar-style YugabyteDB release numbers (`2024.2.1.0-b1`) are exempt anywhere on the line, not only in example payloads; a 2.x release in a pin-shaped position still warns and should be baselined when deliberate.
...
| References | every `references/…` path **named in `SKILL.md`** resolves — both markdown links and backtick-wrapped mentions like `` `references/foo.md` ``, ignoring fenced code so an illustrative path is not treated as a pointer (error). Links *between* reference files are not checked, so a rename there is on you. Every reference file must be linked or mentioned from `SKILL.md` (warn) — only a path that resolves counts, so fixing a broken link can reveal a second file that nothing points at |
