...
description: Retrieve and analyse YugabyteDB universe metrics to identify and explain operational issues — a quick health-check gate, a guided expert workload sweep (read/write mix, active-connections-per-core sizing, hot-table review), and playbooks for issues such as hotspots, high CPU, high memory, I/O saturation, latency/throughput regressions, connection skew, master/catalog RPC pressure, and tablet limits / too many tablets. Use when a user asks why a universe is slow, unbalanced, or unhealthy (or simply "is it OK?"); wants a broad workload/universe metrics review or to inspect tserver/master/node metrics; mentions Prometheus, PromQL, the YBA metrics dashboard, node hotspots, one node hotter than others, uneven connections, connection churn or pool sizing, catalog cache misses or master overload, OOMs, memory pressure rejections, CPU saturation, compaction/SST pressure, too many tablets, the tablet replica limit, or oversized tablets that won't split. Covers picking a metrics source (YugabyteDB Anywhere → Prometheus/PromQL → direct node scrape), finding metrics and per-issue analysis playbooks. Not for YugabyteDB Aeon (limited metrics API) — prefer Performance Advisor there.
---
